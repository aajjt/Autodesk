...
const bucketKey2 = FORGE_CLIENT_ID.toLowerCase() + '_tutorial_bucket2';
...
app.get('/api/forge/createBucket', function (req, res) {
    // Create an application shared bucket using access token from previous route
    // We will use this bucket for storing all files in this tutorial
    Axios({
        method: 'POST',
        ...
        .then(function (response) {
            // Success
            console.log(response);
            res.redirect('/api/forge/detailBucket');
        })
        .catch(function (error) {
            if (error.response && error.response.status == 409) {
                console.log('Bucket already exists, skip creation.');
...
// Route /api/forge/datamanagement/bucket/detail
app.get('/api/forge/detailBucket', function (req, res) {
    Axios({
        method: 'GET',
        url: 'https://developer.api.autodesk.com/oss/v2/buckets/' + encodeURIComponent(bucketKey2) + '/details',
        headers: {
            Authorization: 'Bearer ' + access_token
        }
    })
        .then(function (response) {
            // Success
            console.log(response);
            res.redirect('/api/forge/upload2');
        })
        .catch(function (error) {
            // Failed
            console.log(error);
...
app.get('/api/forge/upload2', function (req, res) {
    
        Axios({
            method: 'PUT',
            url: 'https://developer.api.autodesk.com/oss/v2/buckets/' + encodeURIComponent(bucketKey2) + '/objects/' + encodeURIComponent(fileName),
            headers: {
                Authorization: 'Bearer ' + access_token,
                'Content-Disposition': fileName,
                'Content-Length': fileLength
            },
            data: globalFileContent
        })
            .then(function (response) {
                // Success
                console.log(response);
                var urn = response.data.objectId.toBase64();
                //res.send('successsss');
                res.redirect('/api/forge/translate/' + urn);
            })
            .catch(function (error) {
                // Failed
                console.log(error);
    ...
    app.get('/api/forge/translate/:urn', function (req, res) {
    var urn = req.params.urn;
    var format_type = 'obj';
    ...
    Axios({
        ...
        .then(function (response) {
            ...
            //res.send('translate1 success');
            res.redirect('/api/forge/checkTranslate/' + urn);
        })
        .catch(function (error) {
            // Failed
            console.log(error);
...
app.get('/api/forge/checkTranslate/:urn', function (req, res) {
    var urn = req.params.urn;
    Axios({
        method: 'GET',
        ...
        .then(function (response) {
            // Success
            console.log(response);
            var derivativeUrn= encodeURIComponent(response.data.derivatives[0].children[0].urn);
            //res.send('status check success');
            res.redirect('/api/forge/'+ urn + '/download/' + derivativeUrn);
            
        })
        .catch(function (error) {
            // Failed
            console.log(error);
            res.send('Failed to check status of translation');
        });
});

      app.get('/api/forge/:urn/download/:derivativeUrn', function (req, res) {
        var urn = req.params.urn;
        var derivativeUrn = req.params.derivativeUrn;
        Axios({
            method: 'GET',
            ...
            .then(function (response) {
                // Success
                console.log(response);
                res.attachment('convertedFile.obj');  
